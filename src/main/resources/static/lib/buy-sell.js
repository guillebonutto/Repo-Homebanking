...
var app = new Vue({
	...
	methods: {
		...
		buy() {
			let comprarrrr = this.dolarCompra
			...
			if (!this.buying) {
				...
				this.dollarAccounts = this.accounts.filter(
					(type) => type.type == 'DOLLARS_ACCOUNT'
				)
				if (this.dollarAccounts.length > 0) {
					this.dollarAccounts = this.dollarAccounts.filter(
		...
		sell() {
			if (!this.delling) {
				...
				this.originAccount = this.accounts.filter(
					(type) => type.type == 'DOLLARS_ACCOUNT'
				)
				this.destinyAccount = this.accounts.filter(
